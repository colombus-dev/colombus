...
export async function getNodesFromNeo4J(profilesNames?: string[]) {
	...
	return await axios
		.post<GetNodesFromNeo4JResponse>(
			...
			{
				statement: `MATCH (p:Profile)-[psa]->(sa:Stage)-[sase]->(se:Step)-[sem]->(m:MetaInstruction)-[mc]->(c:Code) OPTIONAL MATCH (sa)-[sasp:PRECEDES]->(:Stage) OPTIONAL MATCH (se)-[sesp:PRECEDES]->(:Step) OPTIONAL MATCH (m)-[mp:PRECEDES]->(:MetaInstruction) OPTIONAL MATCH (c)-[cp:PRECEDES]->(:Code) MATCH (sa)-[cosase:CONTAINS]->(:Step) WITH p, sa, se, m, c, psa, sase, sem, mc, sasp, sesp, mp, cp, count(cosase) AS count_sa_se_children MATCH (se)-[cosem:REFERS_TO]->(:MetaInstruction) WITH p, sa, se, m, c, psa, sase, sem, mc, sasp, sesp, mp, cp, count_sa_se_children, count(cosem) AS count_se_m_children WHERE ANY (name in p.name WHERE name IN ${JSON.stringify(profilesNames)}) RETURN p, sa, se, m, c, psa, sase, sem, mc, sasp, sesp, mp, cp, count_sa_se_children, count_se_m_children ORDER BY elementId(p) DESC, elementId(sa) DESC, elementId(se) DESC, elementId(m) DESC, elementId(c) DESC`,
			},
			config,
		)
		.then(

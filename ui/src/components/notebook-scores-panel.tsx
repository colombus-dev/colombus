...
import type { NotebookScore } from "@/lib/types";
...
function scoreToBandColor(index: number) {
	const colors = ["#ef4444", "#f59e0b", "#facc15", "#a7f3d0", "#22c55e"];
...
function scoreToColor(score: number) {
	const value = clampScore(score);
	const colors = ["#ef4444", "#f59e0b", "#facc15", "#a7f3d0", "#22c55e"];
	const segment = Math.min(colors.length - 2, Math.floor(value * (colors.length - 1)));
	const segmentStart = segment / (colors.length - 1);
	const segmentEnd = (segment + 1) / (colors.length - 1);
	const ratio = segmentEnd > segmentStart ? (value - segmentStart) / (segmentEnd - segmentStart) : 0;
	...
	const start = parseHex(colors[segment]);
	const end = parseHex(colors[segment + 1]);
...
export function NotebookScoresPanel({ notebooks }: NotebookScoresPanelProps) {
	...
	const { slices, averageScore, totalCount } = useMemo(() => {
		...
		const scoreBandStarts = [0, 0.2, 0.4, 0.6, 0.8];

		return {
			slices: buckets
				.map((count, index) => ({
					index,
					bandStart: scoreBandStarts[index],
					count,
					color: scoreToColor(count > 0 ? scoreTotals[index] / count : scoreBandStarts[index] + 0.1),
	...
	const heatmapXAxisLabels = Array.from({ length: heatmapColumns }, (_, index) => (index / (heatmapColumns - 1)).toFixed(2));

	const heatmapGrid = useMemo(
		() =>
			Array.from({ length: heatmapRows }, (_, rowIndex) =>
				Array.from({ length: heatmapColumns }, (_, columnIndex) => {
					const scoreBandIndex =
						columnIndex < 2 ? 0 : columnIndex < 4 ? 1 : columnIndex < 7 ? 2 : columnIndex < 9 ? 3 : 4;
					const colors = ["#ef4444", "#f59e0b", "#facc15", "#a7f3d0", "#22c55e"];
					const rowFactor = (heatmapRows - rowIndex) / heatmapRows;
					const colFactor = (columnIndex + 1) / heatmapColumns;
					return {
						color: colors[scoreBandIndex],
						opacity: Math.max(0.18, Math.min(0.96, 0.28 + rowFactor * 0.36 + colFactor * 0.24)),
					};
				}),
			),
		[]
		...
		>
			...
			<div className="mt-3 grid min-h-0 flex-1 gap-4 lg:grid-cols-[minmax(0,420px)_minmax(0,1fr)] lg:items-start">
					...
					<div className="w-full rounded-2xl border border-slate-200 bg-white/70 p-4 shadow-[0_10px_30px_rgba(15,23,42,0.04)]">
						...
						<div className="flex flex-col items-center gap-4 lg:flex-row lg:items-center lg:justify-center lg:gap-6">
							<div className="relative flex h-[176px] w-[176px] items-center justify-center">
								<div className="absolute inset-0 rounded-full" style={{ background: conicGradient, boxShadow: "inset 0 0 0 1px rgba(148,163,184,0.20)" }} />
								<div className="absolute rounded-full bg-white shadow-[0_0_0_1px_rgba(148,163,184,0.12)]" style={{ width: 116, height: 116 }} />
								<div className="absolute inset-0 flex flex-col items-center justify-center rounded-full">
									<div className="text-[10px] font-semibold uppercase tracking-[0.14em] text-slate-400">Moyenne</div>
									<div className="text-2xl font-semibold text-slate-900">{averageScore.toFixed(2)}</div>
								</div>
							</div>
									...
									<div key={slice.index} className="min-w-0 rounded-xl border border-slate-200 bg-white px-3 py-2.5 shadow-[0_4px_18px_rgba(15,23,42,0.03)]">
										<div className="flex items-start justify-between gap-3">
											<div className="min-w-0 flex-1">
												<div className="text-[11px] font-semibold uppercase tracking-[0.08em] text-slate-500 whitespace-nowrap">{slice.label}</div>
						...
						<div className="flex justify-center">
							<svg viewBox={`0 0 ${radarSize} ${radarSize}`} className="h-[160px] w-[160px] max-w-full overflow-visible">
								...
								{radarPoints.length > 0 && (
									<polygon points={radarPolygonPoints} fill="url(#radar-fill-notebook-scores)" stroke="#0f172a" strokeWidth="2" strokeLinejoin="round" />
								)}

								{radarPoints.map((point, index) => (
									<g key={`point-${index}`}>
										<circle cx={point.x} cy={point.y} r="4.2" fill={radarSlices[index]?.color ?? "#0f172a"} stroke="#ffffff" strokeWidth="2" />
										<text x={axisPoints[index].x} y={axisPoints[index].y} dy={axisPoints[index].y < radarCenter ? -10 : 18} textAnchor="middle" fill="#475569" fontSize="10" fontWeight="600">
				...
				<div className="h-fit self-start rounded-2xl border border-slate-200 bg-white/70 p-4 shadow-[0_10px_30px_rgba(15,23,42,0.04)]">
					...
					<div className="overflow-hidden rounded-2xl border border-slate-200 bg-slate-50 p-[15px]">
						<div className="grid items-start gap-3" style={{ gridTemplateColumns: "36px minmax(0, 1fr)" }}>
							...
							<div className="flex w-full flex-col overflow-hidden rounded-xl border border-slate-200 bg-white/80 p-3">
								<div className="grid w-full gap-[2px]" style={{ gridTemplateColumns: `repeat(${heatmapColumns}, minmax(0, 1fr))`, gridAutoRows: "42px" }}>
									{heatmapGrid.flatMap((row, rowIndex) =>
										row.map((cell, columnIndex) => (
											<div key={`${rowIndex}-${columnIndex}`} className="h-9 w-full rounded-[3px] border border-white/70" style={{ backgroundColor: cell.color, opacity: cell.opacity }} />
										)),
									)}
								</div>
					...
					<div className="mt-4">
						<div className="mb-2 text-[13px] font-semibold text-slate-900">Notebooks</div>
						<ScrollArea className={cn("max-h-[24rem] pr-1") }>
							<div className="space-y-1">
								{notebookRows.map((item) => (
									<div key={item.id} className="flex w-full items-center gap-2 rounded-md px-1 py-1 text-left">
										...
										<span className="shrink-0 text-[10px] font-bold text-slate-900">{item.score.toFixed(2)}</span>
									</div>
								))}
							</div>
							<ScrollBar />

...
export function HomePage({
	...
	const filteredProjectCards = useMemo(() => {
		...
		if (!query) return activeProjectList;
		return activeProjectList.filter(project =>
			[project.name, project.branch, project.description].join(' ').toLowerCase().includes(query)
		);
	}, [activeProjectList, searchQuery]);

	const projectCards = searchQuery ? filteredProjectCards : filteredProjectCards.slice(0, 6);

	return (
		<div className="flex min-h-screen flex-col bg-[linear-gradient(180deg,#f8fafc_0%,#eef2ff_100%)] text-slate-900">
			<WorkspaceHeader
				variant="home"
